...
function overviewPage() {
  return {
    ...
    providerTooltip(p) {
      if (p.health === 'cooldown') return p.display_name + ' \u2014 冷却中（速率限制）';
      if (p.health === 'open') return p.display_name + ' \u2014 断路器已开启';
      if (p.auth_status === 'configured') return p.display_name + ' \u2014 就绪';
      return p.display_name + ' \u2014 未配置';
    },
    ...
    get setupChecklist() {
      return [
        { key: 'provider', label: '配置一个 LLM 提供商', done: this.configuredProviders.length > 0, action: '#settings' },
        { key: 'agent', label: '创建您的第一个智能体', done: (Alpine.store('app').agents || []).length > 0, action: '#agents' },
        { key: 'chat', label: '发送您的第一条消息', done: localStorage.getItem('of-first-msg') === 'true', action: '#chat' },
        { key: 'channel', label: '连接一个消息渠道', done: this.channels.length > 0, action: '#channels' },
